	...
	protected void onDraw(Canvas canvas)
	{
		...
		if (null == bitmap || bitmap.isRecycled()){
			...
			if (drawable != null){
				...
				float scale = 1.0f;
				//创建新画布,关联bitmap,之后画在canvas画布上的图像将会保存到此bitmap对象中,bitmap称为位图，一般位图的文件格式后缀为bmp
				 ...
				 * 即: Canvas canvas = new Canvas(bitmap); drawable.draw(canvas);
				 */
				drawable.draw(drawCanvas); //将图片画到空的Bitmap对象上
				if (mMaskBitmap == null || mMaskBitmap.isRecycled()){
					mMaskBitmap = getBitmap();  //获取图片背景的Bitmap,根据传入的type参数决定是圆角矩形还是圆形底片背景
				}
				// Draw Bitmap.
				mPaint.reset();
				...
				//将准备好的bitmap绘制出来
				canvas.drawBitmap(bitmap, 0, 0, null); //两个canvas对象实现,分别在上面画底色mMaskBitmap和图像Bitmap对象
